...
module.exports.run = async (MAIN, has_iv, target, sighting, internal_value, time_now, area, server, timezone, content, embed) => {
  ...
  pokemon.map_img = await MAIN.Static_Map_Tile(MAIN, sighting.latitude, sighting.longitude, 'pokemon'),
  pokemon.tile = 'https://static-maps.yandex.ru/1.x/?lang=en-US&ll='+sighting.longitude+','+sighting.latitude+'&z=15&l=map&size=400,220&pt='+sighting.longitude+','+sighting.latitude+',pm2rdl'
  ...
  if(has_iv == false || (sighting.cp == null && MAIN.config.POKEMON.sub_without_iv == 'ENABLED')) {
    ...
    if(member){
      ...
      if(content){ content += ' '+pokemon.name+' in '+area.embed+', '+pokestop.mins+'min'}
