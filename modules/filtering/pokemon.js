...
module.exports.run = async (MAIN, sighting, area, server, timezone, role_id) => {
  ...
  MAIN.Pokemon_Channels.forEach((pokemon_channel,index) => {
    ...
    let embed = pokemon_channel[1].embed ? pokemon_channel[1].embed : 'pokemon_iv.js';
    ...
    if(geofences.indexOf(server.name) >= 0 || geofences.indexOf(area.main) >= 0 || geofences.indexOf(area.sub) >= 0){
      ...
      switch(true){
        ...
        case target != 'True':
          ...
          //------------------------------------------------------------------------------
          criteria.gender = target.gender == undefined ? 'all' : target.gender.toLowerCase(),
          criteria.size = target.size == undefined ? 'all' : target.size.toLowerCase(),
          criteria.min_iv = target.min_iv == undefined ? 0 : target.min_iv,
          criteria.max_iv = target.max_iv == undefined ? 100 : target.max_iv,
          criteria.min_cp = target.min_cp == undefined ? 0 : target.min_cp,
          criteria.max_cp = target.max_cp == undefined ? 10000 : target.max_cp,
          criteria.min_level = target.min_level == undefined ? 0 : target.min_level,
          criteria.max_level = target.max_level == undefined ? 35 : target.max_level
          ...
          switch(true){
            ...
            case (criteria.size != 'all' && filter.size != sighting.size): break;
            default:
              switch(true){
        ...
        default:
          ...
          criteria.min_cp = filter.min_cp == undefined ? 0 : filter.min_cp,
